...
//		System.out.println(ArrayUtils.isEmpty(removeNull(data)));
	...
	/**
	 * 计算同一个操作两次操作间隔时间是否达到要求时间(毫秒)
	 * 如果是第一次则记录操作时间，如果达到了间隔时间也记录时间点
	 * @param rid
	 * @param attriName 设置到连接中时的名字
	 * @param time 要求的时间 单位：豪秒
	 * @return boolean true 达到间隔时间 false 没有达到间隔时间
	 * @author subo
	 */
	/*public static boolean isOverSpaceTime(final Session session,final String attriName,final int time){
		if(session==null) return false;
		long gaptime =-1;
		final Object gapObj = session.getAttribute(attriName);
		if(gapObj!=null){
			long gapObjtime = Long.parseLong(gapObj.toString());
			gaptime = System.currentTimeMillis()-gapObjtime;
		}
		if(gaptime>-1&&gaptime<time)return false;
		session.addAttribute(attriName,System.currentTimeMillis());
		return true;
	}*/
...
/*********************************** 身份证验证结束 ****************************************/
	 ...
	 */
	public static boolean isOverSevenDay(){
		int gaptime = CommonUtils.caclBetweenTimes(GamePropertiesCache.serverOpenTime,0);
		return gaptime>7*24*60*60;
	}
	
	/**
	 * 判断是否超过了开服day天了
	 * @param day
	 * @return true 是的  false 没有
	 */
	public static boolean isOverOpenTimeDays(int day){
		int gaptime = CommonUtils.caclBetweenTimes(GamePropertiesCache.serverOpenTime,0);
		return gaptime>day*24*60*60;
	}
	...
	public static void explainLogXml(boolean hasBin){
		File file = new File(CommonUtils.USER_DIR+CommonUtils.SYSTEM_SEPARATE+"config"+CommonUtils.SYSTEM_SEPARATE+"logback.xml");
	 ...
	 */
	public static boolean hasNotZeroHandler(int zeroTime){
		return zeroTime>=0&&TimeDateUtil.isLessTodayZeroTime(zeroTime);
	}
	...
	public static final String connect(String params, String postUrl) {
		if(GamePropertiesCache.isTest)return "{\"ret\":1}";
